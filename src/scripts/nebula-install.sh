...
help()
{
  ...
  echo ""
  echo "Options:"
  echo "    -v      nebula version, default: 2.6.1"
  echo "    -c      nebula component, default: all"
  echo "    -l      vm local_ip, default: 127.0.0.1"
...
FLAG_META_SERVER_ADDRESS="--meta_server_address"
...
while getopts :v:c:l:m:h optname; do
...
setup_data_disk()
{
    if [ -d "/datadisks" ]; then
        ...
        local TEMPDISK="/mnt"
        log "[setup_data_disk] Configuring disk $TEMPDISK/nebula/data"
        mkdir -p "$TEMPDISK/nebula/data"
        chown -R nebula:nebula "$TEMPDISK/nebula"
        chmod 755 "$TEMPDISK/nebula"
        DISK_DATA_PATH="$TEMPDISK/nebula/data"
    else
...
register_graph_systemd()
{
  log "[configure_graph_systemd] register nebula-graphd service"
...
register_storage_systemd()
{
  log "[configure_storage_systemd] register nebula-storaged service"
...
register_meta_systemd()
{
  log "[configure_meta_systemd] register nebula-metad service"
...
start_meta_systemd()
{
    ...
    log "[start_graph_systemd] started Nebula Metad"
}
